...
`crates/reeve-pact/tests/fixtures/test-fixtures.yaml` — included via `#[cfg(test)]`. Lives under the test crate's own `tests/` tree (not workspace `pacts/`) so reviewers and any future `pacts/**` distribution glob never see it as production policy:
