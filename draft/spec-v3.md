...
- Caller may pass `--pact <file>`, `--pact-stdin`, `--allow-preset <name>`
  to scope each run.
- Caller may pass `--no-check` to skip pact validation entirely — all
  `exec()` calls bypass steps 1–6 of the executor flow and run directly.
  Intended for trusted orchestrators that pre-validate commands upstream or
  need unrestricted subprocess access. Emits a `WARN: pact checks disabled
  (--no-check)` line to stderr and records a `no_check: true` field in the
  `script_start` audit event. Never available on `reeve`.
...
exec(binary, args, opts) flow:
  0. If --no-check (reeve-flex only) → skip steps 1–6; jump to step 7.
...
reeve-flex run <script> --no-check
...
### `--no-check` (reeve-flex only)

When `--no-check` is passed, all pact validation (steps 1–6 of the executor
flow) is bypassed. Layer 1 FS scope (`workspace/`) still applies, audit
logging still runs, and per-exec timeouts and output caps are still enforced.

This flag shifts the full trust assumption to the caller. It is appropriate
when:

- The orchestrator has already validated the command set upstream.
- The script set is tightly controlled (e.g., generated by a trusted CI
  pipeline) and attaching a pact YAML would be redundant overhead.

It is **not** a security feature — it is an operational escape hatch for
trusted callers who own the full execution context.

**Never available on `reeve`.** AI agents calling `reeve` directly cannot
reach this path.
